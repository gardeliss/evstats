...
function switchMakersTab(tab) {
    ...
    // Load data - Convert 'monthly' to 'month' and 'yearly' to 'year' for API
    const apiTimePeriod = tab === 'monthly' ? 'month' : 'year';
    loadMakersData(apiTimePeriod);
}
...
async function loadMakersData(timePeriod) {
    try {
        ...
        console.log('📡 Fetching makers data:', url);
        
        const data = await fetchMakersAPI(url);
        
        if (!data) {
            console.error('❌ No makers data received');
            document.getElementById('makersChart').innerHTML = '<p style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Σφάλμα φόρτωσης δεδομένων</p>';
            document.getElementById('makersLoading').style.display = 'none';
            document.getElementById('makersContent').style.display = 'block';
            return;
        }
        
        console.log('✅ Makers data received:', data);
